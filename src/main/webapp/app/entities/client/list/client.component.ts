...
import { combineLatest, debounceTime, distinctUntilChanged, filter, Observable, Subject, switchMap, tap, isEmpty } from 'rxjs';
...
export class ClientComponent implements OnInit {
  ...
  load(): void {
    ...
    if (this.inputValue !== null)
      this.loadFromBackendQuery().subscribe({
        next: (res: EntityArrayResponseType) => {
          this.onResponseSuccess(res);
        },
      });
  ...
  protected loadFromBackendQuery(): Observable<EntityArrayResponseType> {
    return combineLatest([this.activatedRoute.queryParamMap, this.activatedRoute.data]).pipe(
      tap(([params, data]) => this.fillComponentAttributeFromRoute(params, data)),
      switchMap(() =>
        this.queryTerms.pipe(
          debounceTime(300),
          distinctUntilChanged(),
          switchMap(term => this.backendQuery(term)),
        ),
      ),
    );
  }
  ...
  protected backendQuery(query: String): Observable<EntityArrayResponseType> {
    return this.clientService.backendQuery(query).pipe(tap(() => (this.isLoading = false)));
  }
