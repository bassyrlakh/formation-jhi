...
export class ClientService {
  ...
  backendQuery(query: String): Observable<EntityArrayResponseType> {
    return this.http
      .get<RestClient[]>(`${this.resourceUrl}/search?query=${query}`, { observe: 'response' })
  ...
  protected convertDateFromClient<T extends IClient | NewClient | PartialUpdateClient>(client: T): RestOf<T> {
    return {
      ...client,
      dateOfbirth: client.dateOfbirth?.format(DATE_FORMAT) ?? null,
    };
  }
