...
type RestOf<T extends IFacture | NewFacture> = Omit<T, 'dateFacture' | 'dateEcheance'> & {
...
export class FactureService {
  ...
  protected convertDateFromClient<T extends IFacture | NewFacture | PartialUpdateFacture>(facture: T): RestOf<T> {
    return {
      ...
      dateEcheance: facture.dateEcheance?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restFacture: RestFacture): IFacture {
    return {
      ...
      dateEcheance: restFacture.dateEcheance ? dayjs(restFacture.dateEcheance) : undefined,
    };
  }
