...
/**
 * REST controller for managing {@link com.senelec.formation.domain.Client}.
 */
@RestController
...
public class ClientResource {
     ...
     */
    @GetMapping("/search")
    public ResponseEntity<List<ClientDTO>> getAllClientsByQuery(
        @RequestParam String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to get a page of Clients");
        if (!query.isEmpty()) {
            Page<ClientDTO> page = clientService.findAllByQuery(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
        } else return getAllClients(pageable);
    }
